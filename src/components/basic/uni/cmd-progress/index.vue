        ...
        <view class="cmd-progress-inner" :style="setCircleStyle">
          ...
          <block v-if="showInfo && !custom">
            <text class="cmd-progress-text" :title="setFormat">
              <block v-if="status != 'success' && status != 'exception' && setProgress < this.percentMax">{{setFormat}}</block>
              <!-- #ifdef H5 -->
              ...
              <svg v-if="status == 'success' || setProgress == this.percentMax" viewBox="64 64 896 896" data-icon="check" width="1em"
                height="1em" fill="currentColor" aria-hidden="true" :style="{'color': strokeColor ? strokeColor : ''}">
                <path d="M912 190h-69.9c-9.8 0-19.1 4.5-25.1 12.2L404.7 724.5 207 474a32 32 0 0 0-25.1-12.2H112c-6.7 0-10.4 7.7-6.3 12.9l273.9 347c12.8 16.2 37.4 16.2 50.3 0l488.4-618.9c4.1-5.1.4-12.8-6.3-12.8z"></path>
              </svg>
              <!-- #endif -->
              <!-- #ifndef H5 -->
              <text v-if="status == 'exception' || status == 'success' || setProgress == this.percentMax" :style="setCircleIcon"></text>
              <!-- #endif -->
            </text>
          </block>
          <block v-else>
    ...
    <block v-if="type == 'line'">
      ...
      <block v-if="showInfo && !custom">
        <text class="cmd-progress-text" :title="setFormat">
          ...
          <svg v-if="status == 'success' || setProgress == this.percentMax" viewBox="64 64 896 896" data-icon="check-circle" width="1em"
            height="1em" fill="currentColor" aria-hidden="true" :style="{'color': strokeColor ? strokeColor : ''}">
            <path d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm193.5 301.7l-210.6 292a31.8 31.8 0 0 1-51.7 0L318.5 484.9c-3.8-5.3 0-12.7 6.5-12.7h46.9c10.2 0 19.9 4.9 25.9 13.3l71.2 98.8 157.2-218c6-8.3 15.6-13.3 25.9-13.3H699c6.5 0 10.3 7.4 6.5 12.7z"></path>
          </svg>
          <!-- #endif -->
          <!-- #ifndef H5 -->
          <text v-if="status == 'exception' || status == 'success' || setProgress == this.percentMax" :style="setLineStatusIcon"></text>
          <!-- #endif -->
        </text>
      </block>
      <block v-else>
    ...
    computed: {
      ...
      setCirclePathStyle() {
        ...
        return `stroke: ${this.strokeColor};
				stroke-dasharray: ${(this.setProgress / this.percentMax) * (len - (gapDeg||0))}px, ${len}px;
				stroke-dashoffset: -${(gapDeg||0) / 2}px;
      ...
      setLineStatusIcon() {
        ...
        // 完成进度
        if (this.status == 'success' || this.setProgress >= this.percentMax) {
          currentColor = this.strokeColor || '#52c41a'
          svgToBase =
